...
if [[ "${FROM_CI}" == "true" ]]; then
    ...
    if [[ -n ${DOCKERHUB_USERNAME} ]] && [[ ${DOCKERHUB_PASSWORD} ]]; then
        ...
        docker buildx build --platform linux/arm,linux/arm64,linux/amd64 --push -t "${DOCKERHUB_USERNAME}"/samba:latest .
        docker logout
    fi
    if [[ -n "${GITHUB_TOKEN}" ]]; then
        echo "${GITHUB_TOKEN}" | docker login ghcr.io --username "${GITHUB_USERNAME}" --password-stdin
        docker buildx build --platform linux/arm,linux/arm64,linux/amd64 --push -t ghcr.io/"${GITHUB_USERNAME}"/samba:latest .
        docker logout
    fi    
else
